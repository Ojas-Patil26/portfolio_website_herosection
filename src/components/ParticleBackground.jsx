...
// Particle effect ported verbatim from https://github.com/ewohlken2/BreathDearMedusae
// (packages/medusae). Shaders, colors, opacity and all visual styling are kept
// exactly as the original author intended — only the container is adapted so it
// renders as a non-interactive background layer inside the hero.
...
/**
 * Background layer wrapping the Medusae particle field.
 * Transparent Canvas (no opaque fill) so it sits *above the bg colour, below
 * content*. pointer-events:none keeps it from blocking clicks/hovers — the
 * effect still follows the cursor via a global window listener.
 */
